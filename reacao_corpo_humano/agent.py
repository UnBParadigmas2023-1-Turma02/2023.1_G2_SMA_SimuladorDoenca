...
class Dengue(mesa.Agent):
    ...
    def step(self):
        ...
        if empty_cells and self.random.random() < self.taxa_reproducao:
            ...
            offspring = Dengue(self.model.next_id(), self.model, self.dano_vida, self.taxa_reproducao, self.dano_dengue)
...
class GlobuloBranco(mesa.Agent):
    ...
    def step(self):
        ...
        if empty_cells and self.random.random() < self.taxa_reproducao:
            offspring_pos = self.random.choice(empty_cells)
            offspring = GlobuloBranco(self.model.next_id(), self.model, self.adicao_vida, self.taxa_reproducao, self.dano_globulos)
            self.model.grid.place_agent(offspring, offspring_pos)
            self.model.schedule.add(offspring)
